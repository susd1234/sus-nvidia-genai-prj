...
def get_human_feedback(state: State) -> State:
    ...
    try:
        ...
        if state.get("image"):
            print("\nGenerated Image has been saved as 'output.jpg'")
            state["image"].save("output.jpg")
...
def main():
    try:
        ...
        # Run the workflow
        while True:
            product_desc = input("Enter product description (or 'quit' to exit): ")
            if product_desc.lower() == "quit":
                break
